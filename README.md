...
services:
  ...
  keycloak:
    ...
    environment:
      ...
      KC_HOSTNAME: http://keycloak.127.0.0.1.nip.io:9999
      KC_HOSTNAME_ADMIN: http://keycloak.127.0.0.1.nip.io:9999
  ...
  keycloak-init:
    ...
    entrypoint: /bin/sh
    command: /init-keycloak.sh
    volumes:
      - ./init-keycloak-final.sh:/init-keycloak.sh:ro
    depends_on:
      - keycloak
    networks:
      - default
  ...
  webui:
    ...
    environment:
      ...
      OPENID_PROVIDER_URL: http://keycloak.127.0.0.1.nip.io:9999/realms/master/.well-known/openid-configuration
      OPENID_REDIRECT_URI: http://localhost:3000/oauth/oidc/callback
      WEBUI_AUTH_SIGNOUT_REDIRECT_URL: http://localhost:3000/auth
      OAUTH_CLIENT_ID: open-webui
      OAUTH_CLIENT_SECRET: PJa2eqYvlvRKT7kTEuzGsXpkg3fzPohB
...
i=0
while [ $i -lt 60 ]; do
  HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" http://keycloak:8080/realms/master)
  [ "$HTTP_CODE" = "200" ] && break
  i=$((i+1)) && sleep 2
done
...
curl -s -X POST http://keycloak:8080/admin/realms/master/users \
  ...
  -d '{"username":"mohan","enabled":true,"email":"mohan@example.com"}'

# Set password
curl -s -X PUT http://keycloak:8080/admin/realms/master/users/$USER_ID/reset-password \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"type":"password","value":"mohan","temporary":false}'

# Create OIDC client with post-logout redirect URI
